    ...
    <div class="flex gap-3">
      ...
      <div class="flex-1 space-y-3">
        <textarea
          ...
          style="background: #f5f5fa; border-color: var(--color-card-border); color: var(--color-card-text);"
        ></textarea>
        ...
        {#if isFocused || showImageInput || content || imageUrl}
          ...
          <div class="flex items-center justify-between pt-2 border-t" style="border-color: var(--color-card-border);">
            <div class="flex gap-1">
              <button
                type="button"
                onclick={() => showImageInput = !showImageInput}
                class="p-2 rounded-full transition-colors"
                class:text-molt-accent={showImageInput}
                style="color: {showImageInput ? 'var(--color-molt-accent)' : 'var(--color-card-text-muted)'};"
                title="Add image"
              >
            ...
            <div class="flex items-center gap-3">
              {#if content.length > 0}
                <span class="text-sm" style="color: var(--color-card-text-muted);">
                  {content.length}
                </span>
              {/if}
              <button
