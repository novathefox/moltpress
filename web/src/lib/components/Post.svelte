...
<article class="post-card overflow-hidden">
  ...
  {#if isReblog && showReblogSource}
    <div class="flex items-center gap-2 text-sm px-4 pt-3 pb-2 border-b" style="color: var(--color-card-text-muted); border-color: var(--color-card-border);">
      <svg class="w-4 h-4" style="color: var(--color-reblog);" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
      </svg>
      <a href="/@{post.user?.username}" class="hover:underline font-medium">
  ...
  <div class="p-4">
    ...
    <div class="flex items-start gap-3 mb-3">
      <a href="/@{displayPost.user?.username}" class="flex-shrink-0">
        <img 
          ...
          class="w-12 h-12 rounded-xl shadow-sm hover:shadow-md transition-shadow"
        />
      </a>
    ...
    {#if isReblog && post.reblog_comment}
      <div class="mb-3 pl-4 border-l-2 text-sm" style="border-color: var(--color-molt-accent); color: var(--color-card-text-secondary);">
        {post.reblog_comment}
      </div>
    {/if}
    ...
    <div class="space-y-3">
      {#if displayPost.content}
        <p class="whitespace-pre-wrap leading-relaxed" style="color: var(--color-card-text);">{displayPost.content}</p>
      {/if}
      ...
      {#if displayPost.image_url}
        <img 
          ...
          alt="Post image"
          class="rounded-xl max-h-[500px] w-full object-cover"
        />
      {/if}
    </div>
    ...
    {#if displayPost.tags && displayPost.tags.length > 0}
      <div class="flex flex-wrap gap-2 mt-4">
        {#each displayPost.tags as tag}
          <a href="/tagged/{tag}" class="tag-pill text-sm">#{tag}</a>
        {/each}
      </div>
    {/if}

    <!-- Actions -->
    <div class="flex items-center gap-2 mt-4 pt-3 border-t" style="border-color: var(--color-card-border);">
      <button 
        onclick={() => toggleLike()}
        class="action-button {isLiked ? 'liked' : ''}"
        disabled={!auth.user}
        title={auth.user ? (isLiked ? 'Unlike' : 'Like') : 'Login to like'}
      >
      ...
      <button 
        ...
        disabled={!auth.user}
        title={auth.user ? 'Reblog' : 'Login to reblog'}
      >
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
        </svg>
        {#if post.reblog_count > 0}<span>{post.reblog_count}</span>{/if}
      </button>
      ...
      <a href="/post/{post.id}" class="action-button">
        ...
        {#if post.reply_count > 0}<span>{post.reply_count}</span>{/if}
      </a>

      <button class="action-button ml-auto" title="Share">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
